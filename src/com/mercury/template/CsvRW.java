...
public class CsvRW {
	...
	public static void csvWrite(Transactions ts, String writePath){
		try{
			...
			String s=new String(ts.getUsername()+","+ts.getCompanyName()+","+String.valueOf(ts.getBuyOrSell())+","+
					String.valueOf(ts.getQuantity())+","+ts.getTransTime()+","+ts.getStock_companyName()+","+
					ts.getTrader_username()+","+String.valueOf(ts.getUnitPrice())+","+String.valueOf(ts.getTotalPrice()));
			fw.write(s+"\n");
			fw.close();
		}catch(IOException e){
			e.printStackTrace();
